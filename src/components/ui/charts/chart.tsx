...
export const ChartContainer = <T,>(props: ChartContainerProps<T>) => {
  ...
  return (
    <ChartContext.Provider
      ...
      <div
        class={cn('flex aspect-video justify-center', props.class)}
        data-slot='chart'>
        <Switch>
...
export const ChartCrosshair = <T,>(props: ChartCrosshairProps<T>) => {
  ...
  const template = (d: T, x: number | Date) => {
    ...
    const Component = () =>
      props.template ? props.template({ data: d, x, config }) : null
...
export const ChartTooltipContent = <T, C extends ChartConfig = ChartConfig>(
  ...
  return (
    <div class={cn('grid min-w-32 items-start gap-1.5 text-xs', merge.class)}>
